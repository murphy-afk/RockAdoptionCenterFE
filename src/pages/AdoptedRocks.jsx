...
export default function AdoptedRocks() {
  ...
  useEffect(() => {
    axios.get("http://127.0.0.1:8000/api/rocks").then((res) => {
      const adopted = res.data.data.filter((r) => r.adopted);
      setRocks(adopted);
    });
  }, []);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {rocks.map((rock) => (
              <div
                ...
                <img
                  src={rock.image_url || 'https:picsum.photos/400'}
                  alt={rock.name}
                  className="w-full h-48 object-cover rounded-2xl border-4 border-purple-200 mb-4"/>
